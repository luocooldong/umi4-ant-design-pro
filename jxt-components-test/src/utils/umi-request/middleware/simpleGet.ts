...
export function paramsSerialize(params: any, paramsSerializer: any) {
  ...
  if (params) {
    if (paramsSerializer) {
      ...
      if (isArray(params)) {
        ...
        jsonStringifiedParams = {};
        forEach2ObjArr(params, function (value: any, key: any) {
...
export default function simpleGetMiddleware(ctx: any, next: any) {
  if (!ctx) return next();
