...
export default function parseResponseMiddleware(ctx: any, next: any) {
  let copy: any;
      ...
      if (charset === 'gbk') {
        try {
          return res
            .blob()
            .then(readerGBK)
            .then((d: any) => safeJsonParse(d, false, copy, req));
          //@ts-ignore
        } catch (e:any) {
          throw new ResponseError(copy, e.message, null, req, 'ParseError');
        }
      } else if (responseType === 'json') {
        return res.text().then((d: any) => safeJsonParse(d, throwErrIfParseFail, copy, req));
      }
      try {
