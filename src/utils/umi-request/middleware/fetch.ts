...
function __defaultValidateCache(options: any) {
  const { method = 'get' } = options;
  return method.toLowerCase() === 'get';
}

export default function fetchMiddleware(ctx: any, next: any) {
  if (!ctx) return next();
  ...
  const needCache = validateCache(options) && useCache && isBrowser;
  if (needCache) {
