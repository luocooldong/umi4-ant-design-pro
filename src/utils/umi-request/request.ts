...
import { RequestMethod } from './allType';

// 通过 request 函数，在 core 之上再封装一层，提供原 umi/request 一致的 api，无缝升级
const request = (initOptions = {}): RequestMethod => {
  const coreInstance = new Core(initOptions);
  const umiInstance: any = (url: string, options = {}) => {
  ...
  METHODS.forEach(method => {
    umiInstance[method] = (url: string, options: any) => umiInstance(url, { ...options, method });
  });
...
/**
 * Extend 方法参考了ky, 让用户可以定制配置. initOpions 初始化参数
 *
 ...
 */
export const extend = (initOptions: any) => request(initOptions);
